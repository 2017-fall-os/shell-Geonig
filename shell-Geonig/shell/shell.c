...
// Takes 2 char pointer and return a 1 if they are the same
...
int export(char **args)
{
	...
	char **splitCommand = mytoken(args[1], '=');


	if(compareStr(splitCommand[0], "PS1")){

		char **newPs1 = mytoken(splitCommand[1] , '\"');

		if(newPs1[0] == 0) return 1;

		ps1 = newPs1[0];

	}
	else{
		printf("Please input the correct arguments for export: PS1=\"new value of PS1\" \n ");
...
//Combines 2 char pointers into one.
...
void lsh_loop(char **paths, char** envp)
{
	...
	do {
		...
		for(sizeofPs1 = 0; ps1[sizeofPs1] != 0; sizeofPs1++){
		}

		write(0 , ps1 , sizeofPs1+1);
...
// Function to iterate through the envp list and find the list of paths and return it.
